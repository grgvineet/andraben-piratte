...
/************************** Setup **************************/
void
bswabe_setup( bswabe_pub_t** pub, bswabe_msk_t** msk, int parameters_type)
{
	element_t alpha;
	...
	element_t u_k)
{
	...
	element_set(p_0, (msk->rpt)->coef[0]); 
...
/*	while( *attributes )*/
	{
		...
		element_init_G2(h_rj, pub->p);
		element_init_Zr(rj,   pub->p);
		
 		element_from_string(h_rj, c.attr);
 		element_random(rj);

		element_pow_zn(h_rj, h_rj, rj);
		element_pow_zn(h_rj, h_rj, p_0);

		element_mul(c.d, g_r, h_rj);
		element_pow_zn(c.dp, pub->g, rj);
...
fill_policy( bswabe_policy_t* p, bswabe_pub_t* pub, element_t e)
{
	...
	if( p->children->len == 0 )
	{
		...
		// C_y = g^{q_y(0)}   ---> put it in c
		element_pow_zn(p->c,  pub->g, p->q->coef[0]);
		// C'_y = Hash(p->attr)^{q_y(0)}  -----> put it in cp
...
bswabe_enc( bswabe_pub_t* pub, element_t m, char* policy )
{
	...
	// Generate s and m randomly
 	element_random(m);
 	element_random(s);
 	// Calculate e(g,g)^{alpha*s}
	element_pow_zn(cph->cs, pub->g_hat_alpha, s);
	// And finally CS = m*e(g,g)^{alpha*s}
...
/************************** Delegate **************************/
bswabe_del_prv_t*
bswabe_delegate( bswabe_pub_t* pub, bswabe_msk_t* msk, bswabe_prv_t* prv, char* u_k_c_str, char** attributes, size_t num_attr)
{
	int i = 0;
